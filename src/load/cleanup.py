...
from src.config.settings import STAGING_BUCKET, ARCHIVE_BUCKET

def cleanup_archives(collected_results: Union[Dict, List], merge_result: Dict) -> str:
    """
    Archive processed files from staging to archive.
    ...
    for f in all_files:
        ...
        try:
            ...
            # Copy to archive
            if hook.check_for_key(original_path, bucket_name=STAGING_BUCKET):
                hook.copy_object(
                ...
                logging.warning(f"File not found in staging (skipped): {original_path}")
